...
const taggerDir = path.join(publicDir, "tagger");
...
const backupDir = path.join(root, "tools", "backups", "tagger-before-e1");
...
function backupExistingTaggerFile(fileName) {
  const src = path.join(taggerDir, fileName);
...
const taggerPanel = extractBetween(
  '        <div id="taggerPanel"',
...
const taggerIndex = `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <title>BRMedia Tagger</title>
  ...
  <link id="moduleManifest" rel="manifest" href="/tagger/site.webmanifest?v=20260510-split-e1" />
  <meta name="apple-mobile-web-app-capable" content="yes" />
  <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent" />
  <meta name="apple-mobile-web-app-title" content="Tagger" />
  <link rel="stylesheet" href="/tagger/styles.css?v=20260510-split-e1" />
</head>
<body>
...
${sidebarLinks}
    ...
    <main class="moduleMain">
      <section class="moduleHeroCard">
        <div class="moduleEyebrow" id="moduleEyebrow">BRMedia Tagger</div>
        <h1 class="moduleTitle" id="moduleTitle">Tagger</h1>
        <div class="moduleSubtitle" id="moduleSubtitle">Metadata, artwork, BRMedia custom tags and safe save modes.</div>
        ...
        <div class="moduleComingSoonCard">
          ...
          <div class="moduleComingSoonText">
            <div class="moduleComingSoonTitle" id="moduleStatusTitle">Tag this file</div>
            <div class="moduleComingSoonBody" id="moduleComingSoonBody">
              Tagger can receive Player files, library files or uploads, then save BRMedia metadata and write safe tagged copies.
            </div>
          </div>
        </div>
...
${sharedTail}
  <script src="/tagger/app.js?v=20260510-split-e1"></script>
</body>
</html>
`;
...
backupExistingTaggerFile("index.html");
backupExistingTaggerFile("app.js");
backupExistingTaggerFile("styles.css");

fs.writeFileSync(path.join(taggerDir, "index.html"), taggerIndex, "utf8");
fs.copyFileSync(modulesAppPath, path.join(taggerDir, "app.js"));
fs.copyFileSync(modulesStylesPath, path.join(taggerDir, "styles.css"));

console.log("BRMedia Patch E1 complete.");
console.log("Created/updated:");
console.log(" - server/public/tagger/index.html");
console.log(" - server/public/tagger/app.js");
console.log(" - server/public/tagger/styles.css");
console.log("Old /modules folder left untouched as fallback.");
if (fs.existsSync(backupDir)) {
  console.log(`Existing tagger files backed up to ${path.relative(root, backupDir)}`);
}