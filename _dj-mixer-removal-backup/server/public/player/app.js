...
async function fetchDjRecordingHandoffTrack(recordingId = "", fallback = {}) {
  if (!recordingId) return null;

  const res = await fetch("/dj-mixer/recordings", { cache: "no-store" });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || !Array.isArray(data?.recordings)) return null;

  const recording = data.recordings.find((item) => String(item.id) === String(recordingId));
  if (!recording) return null;

  const streamUrl = recording.finalStreamUrl || recording.finalDownloadUrl || `/dj-mixer/recordings/${encodeURIComponent(recording.id)}/stream?kind=final`;
  if (!streamUrl) return null;

  return {
    id: `djrec-${recording.id}`,
    title: recording.title || fallback.title || "BRMedia DJ Recording",
    artist: recording.artist || fallback.artist || "Upalnite",
    albumArtist: recording.artist || fallback.artist || "Upalnite",
    subtitle: `DJ Recording • ${String(recording.format || fallback.finalFormat || "wav").toUpperCase()}`,
    duration: Number(recording.durationSeconds || 0),
    source: "dj_recording",
    sourceType: "djRecording",
    sourceOnline: true,
    sourceStatus: "online",
    mimeType: recording.mimeType || "audio/*",
    streamUrl,
    downloadUrl: recording.finalDownloadUrl || streamUrl,
    recordingId: recording.id,
  };
}

async function playIncomingDjRecordingFromUrl(request = {}) {
  if (!request.recordingId) return null;
  const track = await fetchDjRecordingHandoffTrack(request.recordingId, request);
  if (!track) return null;
  loadTrackIntoPlayer(track, { openPlayer: true, autoplay: request.autoplay });
  return track;
}
...
function getInitialPlayerTrackRequestFromUrl() {
  try {
    ...
    const recordingId = String(params.get("recordingId") || params.get("djRecordingId") || "").trim();
...
async function handleInitialPlayerTrackHandoffFromUrl() {
  ...
  if (!request.trackId && !request.recordingId) return;

  const track = request.recordingId
    ? await playIncomingDjRecordingFromUrl(request)
    : await playIncomingTrackById(request.trackId, {
  ...
  showBookmarkToast("Player", request.recordingId ? "Could not load that DJ recording" : "Could not find that file in the BRMedia library");
}
