...
const DJ_RECORDING_DATA_DIR = path.join(PLAYER_RUNTIME_STATE_DIR, "dj-recordings");
const DJ_RECORDING_CAPTURE_DIR = path.join(DJ_RECORDING_DATA_DIR, "captures");
const DJ_RECORDING_OUTPUT_DIR = path.join(DJ_RECORDING_DATA_DIR, "final");
const DJ_RECORDING_MANIFEST_PATH = path.join(DJ_RECORDING_DATA_DIR, "recordings-manifest.json");
const DJ_PREP_CACHE_PATH = path.join(PLAYER_RUNTIME_STATE_DIR, "dj-prep-cache.json");
const DJ_PREP_CACHE_LIMIT = 20000;
...
function ensureDjRecordingDirs() {
  ensurePlayerRuntimeStateDir();
  fs.mkdirSync(DJ_RECORDING_CAPTURE_DIR, { recursive: true });
  fs.mkdirSync(DJ_RECORDING_OUTPUT_DIR, { recursive: true });
}

function readDjRecordingManifest(): any[] {
  try {
    ensureDjRecordingDirs();
    if (!fs.existsSync(DJ_RECORDING_MANIFEST_PATH)) return [];
    const parsed = JSON.parse(fs.readFileSync(DJ_RECORDING_MANIFEST_PATH, "utf8"));
...
function writeDjRecordingManifest(items: any[]) {
  ensureDjRecordingDirs();
  fs.writeFileSync(DJ_RECORDING_MANIFEST_PATH, JSON.stringify((items || []).slice(0, 250), null, 2), "utf8");
}

function readDjPrepCacheStore(): any {
  try {
    ensurePlayerRuntimeStateDir();
    if (!fs.existsSync(DJ_PREP_CACHE_PATH)) return { version: 1, updatedAt: 0, items: {} };

    const parsed = JSON.parse(fs.readFileSync(DJ_PREP_CACHE_PATH, "utf8"));
    if (parsed?.items && typeof parsed.items === "object" && !Array.isArray(parsed.items)) {
      return { version: 1, ...parsed, items: parsed.items };
    }

    if (Array.isArray(parsed?.items)) {
      const items: Record<string, any> = {};
      parsed.items.forEach((entry: any) => {
        const id = firstString(entry?.id);
        if (id) items[id] = entry;
      });
      return { version: 1, updatedAt: Number(parsed?.updatedAt || 0), items };
    }
  } catch {}
...
function writeDjPrepCacheStore(store: any) {
  ensurePlayerRuntimeStateDir();
  const rawItems = store?.items && typeof store.items === "object" && !Array.isArray(store.items) ? store.items : {};
  const entries = Object.values(rawItems)
    .filter((entry: any) => firstString(entry?.id))
    .sort((a: any, b: any) => Number(b?.serverUpdatedAt || b?.updatedAt || 0) - Number(a?.serverUpdatedAt || a?.updatedAt || 0))
    .slice(0, DJ_PREP_CACHE_LIMIT);
  ...
  fs.writeFileSync(DJ_PREP_CACHE_PATH, JSON.stringify({ version: 1, updatedAt: Date.now(), items }, null, 2), "utf8");
}

function safeDjPrepCacheText(value: any, fallback = "", max = 900) {
  return String(firstString(value) || fallback || "").slice(0, max);
}

function safeDjPrepPlainObject(value: any) {
...
function getDjPrepCacheEntryId(input: any) {
  return safeDjPrepCacheText(
    input?.id || input?.trackId || input?.itemId || input?.item?.id || input?.item?.locator || input?.item?.path || input?.locator || input?.path,
    "",
    1200
  );
}

function normaliseDjPrepCacheEntry(input: any, previous: any = {}) {
  const id = getDjPrepCacheEntryId(input);
  if (!id) return null;

  const item = input?.item && typeof input.item === "object" ? input.item : {};
  const now = Date.now();
  const updatedAt = Math.max(Number(input?.updatedAt || input?.prep?.updatedAt || previous?.updatedAt || 0), now);

  return {
    ...previous,
    id,
    title: safeDjPrepCacheText(input?.title || item?.title || input?.prep?.title || previous?.title, "", 240),
    artist: safeDjPrepCacheText(input?.artist || item?.artist || item?.albumArtist || input?.prep?.artist || previous?.artist, "", 240),
    locator: safeDjPrepCacheText(input?.locator || item?.locator || item?.path || previous?.locator, "", 1200),
    source: safeDjPrepCacheText(input?.source || item?.source || previous?.source, "", 120),
    duration: Number(input?.duration || item?.duration || previous?.duration || 0),
    prep: safeDjPrepPlainObject(input?.prep || previous?.prep),
    analysis: safeDjPrepPlainObject(input?.analysis || previous?.analysis),
    grid: safeDjPrepPlainObject(input?.grid || previous?.grid),
    cue: safeDjPrepPlainObject(input?.cue || previous?.cue),
    waveform: safeDjPrepPlainObject(input?.waveform || previous?.waveform),
    summary: safeDjPrepPlainObject(input?.summary || previous?.summary),
    updatedAt,
    serverUpdatedAt: now,
  };
}

function buildDjPrepCachePayload() {
  const store = readDjPrepCacheStore();
  const rawItems = store?.items && typeof store.items === "object" && !Array.isArray(store.items) ? store.items : {};
  const items = Object.values(rawItems)
    .filter((entry: any) => firstString(entry?.id))
    .sort((a: any, b: any) => Number(b?.serverUpdatedAt || b?.updatedAt || 0) - Number(a?.serverUpdatedAt || a?.updatedAt || 0));

  const prepById: Record<string, any> = {};
  const analysisById: Record<string, any> = {};
  const gridById: Record<string, any> = {};
  const cueById: Record<string, any> = {};

  items.forEach((entry: any) => {
    const id = firstString(entry?.id);
    if (!id) return;
    if (entry.prep && typeof entry.prep === "object") prepById[id] = entry.prep;
    if (entry.analysis && typeof entry.analysis === "object") analysisById[id] = entry.analysis;
    if (entry.grid && typeof entry.grid === "object") gridById[id] = entry.grid;
    if (entry.cue && typeof entry.cue === "object") cueById[id] = entry.cue;
  });
...
async function handleDjPrepCacheUpsert(req: http.IncomingMessage, res: http.ServerResponse) {
  const body = await readJsonBody(req, 8 * 1024 * 1024).catch(() => ({}));
  const records = Array.isArray(body?.items) ? body.items : [body];
  const store = readDjPrepCacheStore();
  const items = store.items && typeof store.items === "object" && !Array.isArray(store.items) ? store.items : {};
  let saved = 0;

  records.forEach((record: any) => {
    const id = getDjPrepCacheEntryId(record);
    if (!id) return;
    const entry = normaliseDjPrepCacheEntry(record, items[id]);
    if (!entry) return;
    items[id] = entry;
    saved += 1;
  });

  writeDjPrepCacheStore({ ...store, items });
  return json(res, 200, { ...buildDjPrepCachePayload(), saved });
}

async function handleDjPrepCacheClear(req: http.IncomingMessage, res: http.ServerResponse) {
  const body = await readJsonBody(req, 1024 * 1024).catch(() => ({}));
  const ids = Array.isArray(body?.ids)
    ? body.ids.map((id: any) => safeDjPrepCacheText(id, "", 1200)).filter(Boolean)
    : [getDjPrepCacheEntryId(body)].filter(Boolean);
  ...
  writeDjPrepCacheStore({ ...store, items });
  return json(res, 200, { ok: true, removed, count: Object.keys(items).length });
}

function safeDjRecordingToken(value: any, fallback = "BRMedia DJ Recording") {
  return String(firstString(value) || fallback)
    .replace(/[\\/:*?"<>|]+/g, "-")
    .replace(/\s+/g, " ")
    .trim()
...
function normaliseDjRecordingFormat(value: any) {
  const format = String(firstString(value) || "wav").toLowerCase();
  return ["wav", "flac", "mp3"].includes(format) ? format : "wav";
}

function djRecordingMimeType(format: string) {
  if (format === "mp3") return "audio/mpeg";
  if (format === "flac") return "audio/flac";
  return "audio/wav";
}

function djRecordingCaptureExt(name: any, contentType = "") {
  const lowerName = String(firstString(name) || "").toLowerCase();
  if (lowerName.endsWith(".m4a") || contentType.includes("mp4")) return "m4a";
  if (lowerName.endsWith(".aac") || contentType.includes("aac")) return "aac";
  if (lowerName.endsWith(".ogg") || contentType.includes("ogg")) return "ogg";
  return "webm";
}

function djRecordingCompressionLevel(value: any) {
  const text = String(firstString(value) || "5").toLowerCase();
  if (text === "fast") return "2";
  if (text === "balanced") return "5";
  if (text === "high") return "8";
  if (text === "maximum") return "12";
  const num = Math.max(0, Math.min(12, Number.parseInt(text, 10) || 5));
  return String(num);
}

function buildDjRecordingFfmpegArgs(inputPath: string, outputPath: string, query: URLSearchParams) {
  const format = normaliseDjRecordingFormat(query.get("format"));
  const args = ["-y", "-hide_banner", "-i", inputPath, "-vn", "-map", "0:a:0?"];
  const sampleRate = Number.parseInt(String(query.get("sampleRate") || "48000"), 10);
  const channels = String(query.get("channels") || "stereo");

  if (channels === "mono") args.push("-ac", "1");
  else if (channels === "left-only") args.push("-af", "pan=mono|c0=FL");
  else if (channels === "right-only") args.push("-af", "pan=mono|c0=FR");
  else if (channels === "dual-mono") args.push("-af", "pan=stereo|c0=FL|c1=FL");
  else args.push("-ac", "2");
  ...
  const title = firstString(query.get("title"));
  const artist = firstString(query.get("artist"));
  if (title) args.push("-metadata", `title=${title}`);
  if (artist) {
    args.push("-metadata", `artist=${artist}`);
    args.push("-metadata", `album_artist=${artist}`);
  }
  args.push("-metadata", "encoder=BRMedia DJ Mixer");

  if (format === "mp3") {
    const bitrate = Math.max(64, Math.min(320, Number.parseInt(String(query.get("mp3Bitrate") || "320"), 10) || 320));
    args.push("-c:a", "libmp3lame", "-b:a", `${bitrate}k`);
  } else if (format === "flac") {
    const bitDepth = String(query.get("flacBitDepth") || "24") === "16" ? "16" : "24";
    args.push("-c:a", "flac", "-compression_level", djRecordingCompressionLevel(query.get("flacCompression")));
    args.push("-sample_fmt", bitDepth === "16" ? "s16" : "s32");
  } else {
    const bitDepth = String(query.get("wavBitDepth") || "24").toLowerCase();
    args.push("-c:a", bitDepth === "16" ? "pcm_s16le" : bitDepth === "32f" || bitDepth === "float" ? "pcm_f32le" : "pcm_s24le");
  }
...
function runDjRecordingFfmpegFinalise(args: string[]) {
  return new Promise<void>((resolve, reject) => {
    const child = spawn(resolvePreviewFfmpegPath(), args, { windowsHide: true });
    let stderr = "";
...
function parseDjRecordingTracklist(value: any) {
  try {
    const parsed = JSON.parse(String(firstString(value) || "{}"));
    const tracks = Array.isArray(parsed?.tracks) ? parsed.tracks : [];
    return {
      ...parsed,
      tracks: tracks.slice(0, 100).map((track: any, index: number) => ({
        index: index + 1,
        id: firstString(track?.id) || "",
        title: firstString(track?.title) || "Unknown title",
        artist: firstString(track?.artist) || "Unknown artist",
        bpm: firstString(track?.bpm) || "",
        key: firstString(track?.key) || "",
        source: firstString(track?.source) || "",
        duration: Number(track?.duration || 0),
        role: firstString(track?.role) || "main",
        note: firstString(track?.note) || "",
        seconds: Number(track?.seconds || 0),
        time: firstString(track?.time) || "",
        loaded: Boolean(track?.loaded),
        planned: Boolean(track?.planned),
        deck: firstString(track?.deck) || "",
      })),
    };
  } catch {
    return { tracks: [] };
...
function formatDjTimestamp(seconds: any) {
...
function formatDjTracklistTxt(title: string, artist: string, tracklist: any, entry: any) {
  const lines = [
    title,
    `Mixed by ${artist}`,
    `Created: ${new Date(Number(entry.createdAt || Date.now())).toLocaleString()}`,
    `Format: ${String(entry.format || "wav").toUpperCase()}`,
    "",
    "Tracklist:",
  ];

  const tracks = Array.isArray(tracklist?.tracks) ? tracklist.tracks : [];
  if (!tracks.length) lines.push("No tracks were captured in the session list yet.");
  tracks.forEach((track: any, index: number) => {
    const meta = [track.bpm ? `${track.bpm} BPM` : "", track.key && track.key !== "—" ? `Key ${track.key}` : ""].filter(Boolean).join(" • ");
    const time = typeof track.time === "string" && track.time ? track.time : formatDjTimestamp(track.seconds);
    const role = track.role && track.role !== "main" ? ` [${track.role}]` : "";
    const note = track.note ? ` — ${track.note}` : "";
    lines.push(`${String(index + 1).padStart(2, "0")}. ${time} ${track.artist} - ${track.title}${role}${meta ? ` (${meta})` : ""}${note}`);
  });

  lines.push("", "Generated by BRMedia DJ Mixer");
  return lines.join("\n");
}

function buildDjTimestampJson(title: string, artist: string, tracklist: any, entry: any) {
  const tracks = Array.isArray(tracklist?.tracks) ? tracklist.tracks : [];
  return {
    title,
    artist,
    source: "BRMedia DJ Mixer",
    recordingId: entry.id,
    createdAt: entry.createdAt,
    durationSeconds: Number(entry.durationSeconds || 0),
    linkedSet: tracklist?.linkedSet || null,
    setup: tracklist?.setup || {},
    tracks: tracks.map((track: any, index: number) => {
      const seconds = Math.max(0, Math.floor(Number(track.seconds || 0)));
      return {
        index: index + 1,
        time: typeof track.time === "string" && track.time ? track.time : formatDjTimestamp(seconds),
        seconds,
        title: track.title,
        artist: track.artist,
        bpm: track.bpm || "",
        key: track.key || "",
        role: track.role || "main",
        note: track.note || "",
        source: track.source || "",
      };
    }),
...
function getPublicDjRecordingLibraryItem(item: any) {
  if (!item) return null;
  return {
    id: item.id,
    title: item.title,
    ...
    mimeType: item.mimeType,
    source: item.source,
    sourceOnline: item.sourceOnline,
    sourceStatus: item.sourceStatus,
  };
}

async function ensureDjRecordingLibraryItem(entry: any, title?: string) {
  if (!entry?.finalPath || !fs.existsSync(entry.finalPath)) return null;
  const libraryItem = await addLocalFileToLibraryWithMetadata(entry.finalPath, title || entry.title || path.parse(entry.finalPath).name);
  (libraryItem as any).artist = (libraryItem as any).artist || entry.artist || "Upalnite";
  (libraryItem as any).album = (libraryItem as any).album || entry.linkedSetName || "BRMedia DJ Recordings";
  (libraryItem as any).genre = (libraryItem as any).genre || "DJ Mix";
  (libraryItem as any).comment = (libraryItem as any).comment || "BRMedia DJ Mixer recording";
  entry.libraryItemId = libraryItem.id;
  entry.libraryItem = getPublicDjRecordingLibraryItem(libraryItem);
  entry.finalStreamUrl = `/dj-mixer/recordings/${encodeURIComponent(entry.id)}/stream?kind=final`;
  return libraryItem;
}

function writeDjRecordingSidecars(entry: any, title: string, artist: string, tracklist: any, query: URLSearchParams) {
  const base = path.join(DJ_RECORDING_OUTPUT_DIR, path.parse(entry.finalFilename).name);
  const sidecars: any = {};

  if (query.get("saveTxt") !== "0") {
    const txtFilename = `${path.basename(base)}.tracklist.txt`;
    const txtPath = path.join(DJ_RECORDING_OUTPUT_DIR, txtFilename);
    fs.writeFileSync(txtPath, formatDjTracklistTxt(title, artist, tracklist, entry), "utf8");
    sidecars.txtFilename = txtFilename;
    sidecars.txtPath = txtPath;
    sidecars.txtUrl = `/dj-mixer/recordings/${encodeURIComponent(entry.id)}/download?kind=txt`;
  }

  if (query.get("saveTimestampJson") !== "0") {
    const timestampFilename = `${path.basename(base)}.timestamps.json`;
    const timestampPath = path.join(DJ_RECORDING_OUTPUT_DIR, timestampFilename);
    fs.writeFileSync(timestampPath, JSON.stringify(buildDjTimestampJson(title, artist, tracklist, entry), null, 2), "utf8");
    sidecars.timestampJsonFilename = timestampFilename;
    sidecars.timestampJsonPath = timestampPath;
    sidecars.timestampJsonUrl = `/dj-mixer/recordings/${encodeURIComponent(entry.id)}/download?kind=timestamp`;
  }

  if (query.get("saveSessionJson") !== "0") {
    const sessionFilename = `${path.basename(base)}.session.json`;
    const sessionPath = path.join(DJ_RECORDING_OUTPUT_DIR, sessionFilename);
    fs.writeFileSync(sessionPath, JSON.stringify({ ...tracklist, recording: { ...entry, capturePath: "", finalPath: "" } }, null, 2), "utf8");
    sidecars.sessionFilename = sessionFilename;
    sidecars.sessionPath = sessionPath;
    sidecars.sessionUrl = `/dj-mixer/recordings/${encodeURIComponent(entry.id)}/download?kind=session`;
  }

  return sidecars;
}

async function handleDjRecordingFinalise(req: http.IncomingMessage, res: http.ServerResponse, url: URL) {
  ensureDjRecordingDirs();

  const format = normaliseDjRecordingFormat(url.searchParams.get("format"));
  const id = safeDjRecordingToken(url.searchParams.get("id"), `djrec-${Date.now()}-${crypto.randomBytes(3).toString("hex")}`);
  const title = safeDjRecordingToken(url.searchParams.get("title"), "BRMedia DJ Recording");
  const artist = safeDjRecordingToken(url.searchParams.get("artist"), "Upalnite");
  const captureExt = djRecordingCaptureExt(url.searchParams.get("name"), String(req.headers["content-type"] || ""));
  const finalName = safeDjRecordingToken(url.searchParams.get("finalName") || title, title).replace(/\.(webm|m4a|aac|ogg|wav|flac|mp3)$/i, "");
  const stamped = `${Date.now()}-${crypto.randomBytes(4).toString("hex")}`;
  const captureFilename = `${finalName}-${stamped}.capture.${captureExt}`;
  const finalFilename = `${finalName}-${stamped}.${format}`;
  const capturePath = path.join(DJ_RECORDING_CAPTURE_DIR, captureFilename);
  const finalPath = path.join(DJ_RECORDING_OUTPUT_DIR, finalFilename);

  const buffer = await readBufferBody(req, 1536 * 1024 * 1024);
  if (!buffer.length) return json(res, 400, { ok: false, error: "Empty recording upload" });

  fs.writeFileSync(capturePath, buffer);
  await runDjRecordingFfmpegFinalise(buildDjRecordingFfmpegArgs(capturePath, finalPath, url.searchParams));

  const stat = fs.statSync(finalPath);
  const manifest = readDjRecordingManifest();
  const tracklist = parseDjRecordingTracklist(url.searchParams.get("tracklist"));
  const entry: any = {
    id,
    title,
    artist,
    ...
    finalPath,
    bytes: stat.size,
    durationSeconds: Number(url.searchParams.get("durationSeconds") || 0),
    createdAt: Date.now(),
    trackCount: Array.isArray(tracklist?.tracks) ? tracklist.tracks.length : 0,
    plannedTrackCount: Number(tracklist?.plannedTrackCount || 0),
    loadedTrackCount: Number(tracklist?.loadedTrackCount || 0),
    linkedSetName: tracklist?.linkedSet?.name || "",
    tracklistMode: tracklist?.setup?.tracklistMode || "",
    timestampSource: tracklist?.setup?.timestampSource || url.searchParams.get("timestampSource") || "load-time",
    finalDownloadUrl: `/dj-mixer/recordings/${encodeURIComponent(id)}/download?kind=final`,
    finalStreamUrl: `/dj-mixer/recordings/${encodeURIComponent(id)}/stream?kind=final`,
    captureDownloadUrl: `/dj-mixer/recordings/${encodeURIComponent(id)}/download?kind=capture`,
  };

  entry.sidecars = writeDjRecordingSidecars(entry, title, artist, tracklist, url.searchParams);
  await ensureDjRecordingLibraryItem(entry, title);
  persistAudioLibraryManifest();
  writeDjRecordingManifest([entry, ...manifest.filter((item) => item.id !== id)]);

  appendStatsEvent("dj_recording_finalised", "dj", {
    entityType: "dj_recording",
    entityId: id,
    title,
    status: "done",
    route: "dj-mixer",
    value: Number(entry.durationSeconds || 0),
    extra: {
      format,
      bytes: stat.size,
      finalFilename,
  ...
  return json(res, 200, { ok: true, recording: entry });
}

function findDjRecording(id: string) {
  return readDjRecordingManifest().find((item) => String(item.id) === String(id));
}

async function listDjRecordingsPayload() {
  const manifest = readDjRecordingManifest();
  let changed = false;

  for (const item of manifest) {
    if (!item.libraryItemId && item.finalPath && fs.existsSync(item.finalPath)) {
      try {
        await ensureDjRecordingLibraryItem(item, item.title);
        changed = true;
      } catch (err) {
        console.warn("DJ recording library registration failed", err);
      }
    }
    if (item.id && !item.finalStreamUrl) {
      item.finalStreamUrl = `/dj-mixer/recordings/${encodeURIComponent(item.id)}/stream?kind=final`;
      changed = true;
    }
  }
  ...
  if (changed) {
    persistAudioLibraryManifest();
    writeDjRecordingManifest(manifest);
  }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/dj-mixer/prep-cache") {
      return json(res, 200, buildDjPrepCachePayload());
    }

    if (req.method === "POST" && url.pathname === "/dj-mixer/prep-cache") {
      return await handleDjPrepCacheUpsert(req, res);
    }

    if (req.method === "POST" && url.pathname === "/dj-mixer/prep-cache/clear") {
      return await handleDjPrepCacheClear(req, res);
    }
		
    if (req.method === "GET" && url.pathname === "/dj-mixer/recordings") {
      return json(res, 200, await listDjRecordingsPayload());
    }

    if (req.method === "POST" && url.pathname === "/dj-mixer/recordings/finalise") {
      try {
        return await handleDjRecordingFinalise(req, res, url);
      } catch (err: any) {
    ...
    if (
      ...
      url.pathname.startsWith("/dj-mixer/recordings/") &&
      (url.pathname.endsWith("/download") || url.pathname.endsWith("/stream"))
    ) {
      const isStream = url.pathname.endsWith("/stream");
      const id = decodeURIComponent(url.pathname.replace("/dj-mixer/recordings/", "").replace(/\/(download|stream)$/, "").trim());
      const item = findDjRecording(id);
      if (!item) return json(res, 404, { ok: false, error: "Recording not found" });

      const requestedKind = String(url.searchParams.get("kind") || "final");
      const kind = ["capture", "txt", "timestamp", "session"].includes(requestedKind) ? requestedKind : "final";
      const sidecars = item.sidecars || {};
      const filePath = kind === "capture"
        ? item.capturePath
        : kind === "txt"
          ? sidecars.txtPath
          : kind === "timestamp"
            ? sidecars.timestampJsonPath
            : kind === "session"
              ? sidecars.sessionPath
              : item.finalPath;
      const downloadName = kind === "capture"
        ? item.captureFilename
        : kind === "txt"
          ? sidecars.txtFilename
          : kind === "timestamp"
            ? sidecars.timestampJsonFilename
            : kind === "session"
              ? sidecars.sessionFilename
              : item.finalFilename;

      if (!filePath || !fs.existsSync(filePath)) return json(res, 404, { ok: false, error: "Recording file missing" });

      streamFileWithRange(req, res, filePath, {
        asAttachment: !isStream,
        downloadName,
        mimeType: kind === "capture" ? "application/octet-stream" : kind === "txt" ? "text/plain; charset=utf-8" : kind === "timestamp" || kind === "session" ? "application/json; charset=utf-8" : djRecordingMimeType(item.format),
      });
      return;
    }
  ...
  return;
    ...
    const appManifestRouteMap: Record<string, { startUrl: string; name: string }> = {
      ...
      "/dj-mixer/site.webmanifest": {
        startUrl: "/dj-mixer?v=20260601",
        name: "BRMedia DJ Mixer",
      },
    };
  ...
  return json(res, 404, { error: "Player not found" });
    ...
    const modulePageRouteMap: Record<string, { key: string; title: string; appleTitle: string; manifestHref: string; folder?: string; fallbackToModules?: boolean }> = {
      ...
      "/dj-mixer": {
        ...
        folder: "dj-mixer",
        fallbackToModules: false,
        title: "BRMedia DJ Mixer",
        appleTitle: "DJ Mixer",
        manifestHref: "/dj-mixer/site.webmanifest?v=20260601",
      },
    };
